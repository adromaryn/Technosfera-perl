...
sub start_server {
    ...
    if($receiver_count == $max_receiver) {
			my $data = {
					   header => Local::TCP::Calc->pack_header(TYPE_CONN_ERR(), 2),
					   message => "ok"
				   };
      ...
      if (defined $child){
        ...
        print "Client $host:$service $/";
				my $data = {
						header => Local::TCP::Calc->pack_header(TYPE_CONN_OK(), 2),
						message => "ok"
				};
				my $res = encode_json $data;
        $client ->send($res."\n");
				while(<$client>) {
					...
					if ($size != length ($answer{message})) {
						...
						my $data = {
						  header => Local::TCP::Calc->pack_header(TYPE_CONN_ERR(), length $error),
							message => $error
						};
					  my $res= encode_json $data;
		   		  $client ->send($res."\n");
					} else {
						if ($type == TYPE_CHECK_WORK()) {
						  # Если пришли с проверкой статуса, получаем статус из очереди и отдаём клиенту
							my ($status, $res) = $q->get_status($answer{message});
							if ($status == -1) {
								my $error = "Work not found";
								my $data = {
									header => Local::TCP::Calc->pack_header(TYPE_CONN_ERR(), length $error),
									message => $error
								};
							}
						} elsif($type == TYPE_START_WORK()) {
